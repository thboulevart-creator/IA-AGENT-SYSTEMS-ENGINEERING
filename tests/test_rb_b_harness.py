...
from experiments.rb_b.tools import FaultPlan, WorkspaceTools
...
class RBBHarnessTests(unittest.TestCase):
    ...
    def test_e2_s0_and_s1_do_not_replan_but_s2_repairs(self):
        def corrupt(action, ws, trace):
            if action == "WRITE_ARTIFACT":
                p = ws / "artifact.txt"
                p.write_bytes(p.read_bytes()[:-1] + b"X")
                trace.event("external_mutation", target="artifact.txt")

        s0, _ = self.run_variant("S0", "E2", corrupt)
        s1, _ = self.run_variant("S1", "E2", corrupt)
        s2, trace = self.run_variant("S2", "E2", corrupt)
        self.assertEqual(s0["final_outcome"], "FAIL")
        self.assertEqual(s1["final_outcome"], "FAIL")
    ...
    def test_e4_execution_failure_is_traceable(self):
